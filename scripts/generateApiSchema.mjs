...
// --- Процесс генерации ---
try {
  console.log(`🚀 [${serviceName}] Шаг 1/2: Получение и форматирование схемы...`)
  execSync(`npx openapi-format ${url} -o ${openapiJsonPath} --casingFile ${casingFilePath}`, {
    stdio: 'inherit',
  })

  console.log(`🚀 [${serviceName}] Шаг 2/2: Генерация файла типов schema.ts...`)
  execSync(`npx openapi-typescript ${openapiJsonPath} -o ${schemaTsPath}`, { stdio: 'inherit' })

  console.log(`\n🎉 [${serviceName}] Готово! Файл schema.ts успешно сгенерирован.`)
} catch (error) {
