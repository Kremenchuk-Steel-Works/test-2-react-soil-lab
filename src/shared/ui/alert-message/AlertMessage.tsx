...
const alertConfigs: Record<AlertType, { icon: React.ReactNode; style: string }> = {
  SUCCESS: {
    icon: <CheckCircle className="h-5 w-5 shrink-0 text-green-600 dark:text-green-400" />,
    style:
  ...
  WARNING: {
    icon: <AlertTriangle className="h-5 w-5 shrink-0 text-yellow-600 dark:text-yellow-400" />,
    style:
...
const AlertMessage: React.FC<AlertMessageProps> = ({ type, message, className }) => {
  ...
  return (
    <div className={`${baseStyle} ${style} ${className ?? ''}`} role="alert" aria-live="polite">
      {icon}
